...
namespace Personal_Website.Projects.Minesweeper {
    ...
    public static class Field {
        ...
        public static List<Change> openPlace(int x, int y) {
            Place[][] field = (Place[][])HttpContext.Current.Session["Field"];
			...
			if (!field[y][x].isDiscovered) {
				...
				int placesDiscovered = (int)HttpContext.Current.Session["placesDiscovered"] + result.Count();
        ...
        private static bool isWon() {
			...
			if (((Parameters)HttpContext.Current.Session["Parameters"]).safePlaces == (int)HttpContext.Current.Session["placesDiscovered"] && !(bool)HttpContext.Current.Session["lost"]) {
				...
				// put victory to DB
				if (!(bool)HttpContext.Current.Session["solverUsed"])
					new Thread(delegate() {
						Utility.addAchievment(userId, mode);
					}).Start();

				HttpContext.Current.Session["won"] = true;
				return true;
			} else {
				return false;
			}
        ...
        public static List<Change> getAllMines() {
			...
			if ((bool)HttpContext.Current.Session["lost"]) {
