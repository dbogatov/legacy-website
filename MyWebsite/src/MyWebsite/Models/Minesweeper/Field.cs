...
namespace MyWebsite.Models.Minesweeper {
    ...
    public static class Field {
        ...
        public static List<Change> openPlace(int x, int y) {
            ...
            Place[][] field = httpContext.HttpContext.Session.GetObjectFromJson<Place[][]>("Field");// GetString("Field");
			...
			if (!field[y][x].isDiscovered) {
				...
				if (field[y][x].isMine) {
					...
					Field.lose();
				} else if (field[y][x].number == 0) { // if place is sea, reveal it
					result = revealSea(y, x, field, result);
				} else { // otherwise, just discover it
					field[y][x].isDiscovered = true;
					result.Add(new Change(x, y, field[y][x]));
				}

                httpContext.HttpContext.Session.SetObjectAsJson("Field", field);

                // increase number of opened places
                int placesDiscovered = httpContext.HttpContext.Session.GetInt32("placesDiscovered").Value + result.Count(); // ["placesDiscovered"] + result.Count();

                httpContext.HttpContext.Session.SetInt32("placesDiscovered", placesDiscovered);
                //HttpContext.Current.Session["placesDiscovered"] = placesDiscovered;

                // check, whether player has won
                httpContext.HttpContext.Session.SetInt32("won", Convert.ToInt32(Field.isWon()));
        ...
        private static bool isWon() {
			...
			if (httpContext.HttpContext.Session.GetObjectFromJson<Parameters>("Parameters").safePlaces == httpContext.HttpContext.Session.GetInt32("placesDiscovered") && !Convert.ToBoolean(httpContext.HttpContext.Session.GetInt32("lost").Value)) {
				System.Diagnostics.Debug.Write("Win!");

                int userId = httpContext.HttpContext.Session.GetInt32("userID").Value;
                int mode = httpContext.HttpContext.Session.GetObjectFromJson<Parameters>("Parameters").mode;

                //int mode = ((Parameters)HttpContext.Current.Session["Parameters"]).mode;

                // put victory to DB
                if (!Convert.ToBoolean(httpContext.HttpContext.Session.GetInt32("solverUsed").Value))
					new Thread(delegate() {
						Utility.addAchievment(userId, mode);
					}).Start();

				httpContext.HttpContext.Session.SetInt32("won", Convert.ToInt32(true));
				return true;
			} else {
				return false;
			}
		...
		private static void lose() {
			httpContext.HttpContext.Session.SetInt32("lost", Convert.ToInt32(true));
		}
        ...
        public static List<Change> getAllMines() {
			Place[][] field = httpContext.HttpContext.Session.GetObjectFromJson<Place[][]>("Field");
			List<Change> result = new List<Change>();

            

            if (Convert.ToBoolean(httpContext.HttpContext.Session.GetInt32("lost").Value)) {
